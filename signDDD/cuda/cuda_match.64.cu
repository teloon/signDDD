...
//	unsigned int frame_num_query =1068;//number of frames in query
	...
	unsigned int mem_size_lib = frame_num_index * sizeof(unsigned long);//the memory size hvectors_in_lib needed
	...
	// Device variables
	unsigned long* di_index;//input data in gpu which contains all feature vectors of frames
	unsigned long* di_query;//input data in cpu which contains all feature vectors of query frames
	unsigned long* do_result;//the result in device
...
//	}
	...
	for(int i = 0 ; i < frame_num_query ; i++)
	{
		...
		for(int j = 0; j < ho_result_cnt[i]; j++)
			fprintf(fp2,"%lu ",ho_result[i*MAX_SIM_NUM+j]);
		fprintf(fp2,"\n");
	}
